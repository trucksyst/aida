...
- Добавлен `AI Block` в `background/ai-block.js`.
...
3. В `background/ai-block.js` реализован вызов `chrome.identity.getAuthToken()`.
...
6. AI Block отправляет запрос в Vertex AI через `fetch()` и `Bearer` token.
