...
**Единый стандарт данных — по образцу DAT.** Все борды приводятся к одному формату; лишних парсеров и своих нормализаторов под каждый борд не делаем — один контракт данных, адаптер каждого борда только переводит ответы и запросы своего API в этот формат и обратно. Единый формат по DAT касается **только формата карточки груза и операций worklist**; протокол сообщений между UI и Core (GET_LOADS, DATA_UPDATED и т.п.) к стандарту DAT не относится.

Адаптер берёт токен из Storage, формирует запрос к API борда (GraphQL для DAT, REST для Truckstop и т.д.) и возвращает результат в **едином формате (по DAT)**.

### Единый формат карточки груза (по DAT)
...
```js
{
  ...
  comments:    string,          // описание груза (COMMENTS с борда) — обязательно на всех карточках
