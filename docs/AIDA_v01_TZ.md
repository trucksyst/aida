...
| AI Block  | `background/ai/` + `auth/`   | Core, UI (чат)     | AI-анализ, чат-интенты, рекомендации действий, AI OAuth |
...
- Авторизация AI живёт отдельно от бизнес-логики AI в `auth/auth-ai.js` и использует только `OAuth` (`chrome.identity.getAuthToken`), без `API key`.
...
### Архитектурное правило AI

- `auth` находится в `auth/` и не смешивается с AI business logic.
- AI не ходит напрямую в адаптеры бордов.
- Поиск и любые другие действия AI исполняются только через Core API и стандартный pipeline AIDA.
- AI читает только нормализованный контекст AIDA: `settings`, `lastSearch`, `work:loads` и будущие AI rules/profile namespaces.
- UI не должен содержать бизнес-логику AI-провайдера; UI отвечает только за чат, визуализацию и отправку команд в Core.

### Целевая структура AI
...
- `auth/auth-ai.js`
  ...
  - статус AI OAuth, connect/disconnect и очистка кэша
- `background/ai/`
  - вызов Vertex AI `generateContent` через `fetch()`
  - chat/analyze use-cases
...
3. `auth/auth-ai.js` запускает `chrome.identity.getAuthToken({ interactive: true })`.
...
8. `background/ai/provider.js` вызывает Vertex endpoint `generateContent`.
