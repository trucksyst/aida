...
### Единый формат карточки груза (максимальный контракт)
...
```js
{
  ...
  externalId:    string,          // ID на борде (postingId, external_id, shipment_id)
  referenceId:   string,          // доп. референс (reference_id, postersReferenceId)

  // === Маршрут — Origin ===
  origin: {
    city:        string,
    state:       string,          // 2-буквенный код (IL, TX, CA)
    zip:         string,
    lat:         number | null,   // координаты
    lng:         number | null,
  },
  // === Маршрут — Destination ===
  destination: {
  ...
  // === Груз ===
  equipment:     string,          // 'VAN' | 'REEFER' | 'FLATBED' | 'STEPDECK' и т.д.
  equipmentCode: string,          // оригинальный код борда
  equipmentAll:  string[],        // все типы если несколько (TP: ["stepdeck","flatbed"])
  weight:        number | null,   // lbs
  length:        number | null,   // ft
  width:         number | null,   // ft
  loadSize:      string,          // 'full' | 'partial' | ''

  // === Расстояние и цена ===
  miles:         number | null,
  deadhead:      number | null,   // миль deadhead до pickup
  rate:          number | null,   // $ total
  rpm:           number | null,   // $ per mile
  avgRate:       number | null,   // средняя рыночная цена (avg_price, estimatedRatePerMile)

  // === Брокер / Компания ===
  broker: {
    name:        string,          // company || companyName || contact_name
    contactName: string,          // имя контактного лица (contact_name)
    ...
    rating:      number | null,   // credit score / transcredit rating
    daysToPay:   number | null,   // среднее кол-во дней до оплаты
  },

  // === Описание ===
  comments:      string,          // description || comments — заметки к грузу (Note)

  // === Даты ===
  pickupDate:    string,          // ISO date (только дата)
  pickupTime:    string,          // время пикапа если есть
  deliveryDate:  string,          // дата доставки
  postedAt:      string,          // ISO datetime — когда опубликовано

  // === Статус и мета ===
  status:        'active',
  statusUpdatedAt: string,
  receivedAt:    string,          // когда получено AIDA

  // === Источник / канал ===
  channel:       string,          // канал (BrokerDirect, TMSLayer, LBN)
  source:        string,          // drill-down источник (jakebrake, LoadBoardNetwork)

  // === Флаги ===
  bookNow:       boolean,         // доступна мгновенная бронь
  biddable:      boolean,         // можно делать ставку
  isFactorable:  boolean,         // доступен факторинг
  isNew:         boolean,         // новый груз

  // === Оригинал ===
  raw:           object           // полный оригинальный ответ борда (страховка)
}
```

> **Правило:** при добавлении нового борда — если у него есть поле, которого нет в контракте, контракт расширяется. Поля НИКОГДА не удаляются из контракта.
