...
> Дата: 2026-03-06
> Статус: УТВЕРЖДЁН (TP — в перспективу)
...
**Результат:**
- `background.js`: ~1200 строк → ~300 строк
- Добавление нового борда = 1 файл адаптера + 1 строка регистрации
...
### Фаза 0: Подготовка
- [ ] Создать snapshot/бэкап текущего рабочего кода (git tag `pre-autonomy`)
- [ ] Убедиться что всё работает как есть (smoke test)
